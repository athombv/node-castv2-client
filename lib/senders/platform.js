...
PlatformSender.prototype.connect = function(options, callback) {
  ...
  if( !self.client ) return console.warn("Missing self.client");
  ...
  this.client.connect(options, function() {
    ...
    function onclose() {
      self.emit('close');
      self.heartbeat && self.heartbeat.stop();
      self.receiver && self.receiver.removeListener('status', onstatus);
      self.receiver && self.receiver.close();
      self.heartbeat && self.heartbeat.close();
